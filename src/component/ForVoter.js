  ...
  const ForVoter = () => {
    ...
    return (
      <div className=" flex flex-col justify-center items-center text-black">
        ...
        {/* Vote To */}
        <div className="flex w-3/4 max-sm:w-52 justify-center items-center mt-5">
          <input value={App.ID} onChange={(e) => App.setID(e.target.value)}  type={"number"} className="w-4/5 max-sm:w-64 max-sm:ml-0 p-3 bg-black border-2 border-white text-white border-opacity-80 bg-opacity-80 outline-none rounded-lg" placeholder="Enter Candidates ID" />
        </div>
        {/* Button */}
        {App.TxLoading ?
